...
void isr_timer(uint32_t irq, struct context *ctx)
{
    ...
    if (g_timer_ticks%(HZ) == 0){//每秒进行一次优先级的检查和更新
      int nready =0;
    ...
    fixedpt r1_60 = fixedpt_div(FIXEDPT_ONE,fixedpt_fromint(60));
    g_load_avg = fixedpt_add(fixedpt_mul(r59_60,g_load_avg),fixedpt_mul(r1_60,nready));
   // printk("g_load_avg is %d",fixedpt_toint(g_load_avg));
        }
    }
  }
}
